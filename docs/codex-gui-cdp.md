...
2. **思考强度是滑块**（`role="slider"`，`aria-valuemin=0` / `aria-valuemax=4`），**不是菜单项**：
   5 档标签依次为 **轻度(0) / 中(1) / 高(2) / 极高(3) / 极高(4)**。
...
回读校验：菜单展开时触发器自身文本读不到，改读菜单内「选择模型」项文本（形如 `GPT-5.6 Sol 高`），
关菜单后再用触发器文本复核；两处都做 UI 重渲染等待（避免读到空值误判）。不符则重试 ≤3 次，
仍不符中止上报 `model_mismatch`。
